...
#    popup.lift()
    #text4.title('Found pattern %s in  %d out of %d lines' % (selection,n,linecount))

    text4.load(tmp+"found.txt")
...
$cheese                 ## Looks for 'cheese' flags, rather than the word itself
 ...
 """
    ...
    label = tkinter.messagebox.showinfo("About", text)
    ...
    with open(tmp+'patterns2.txt', 'r',encoding="utf-8") as patterns:
        for pattern in patterns:
            ...
            if '>' in pattern:
                ...
                if '$' in p[0]: # pattern is $flagname, so only search LHS
                    pp=re.sub('\$','',p[0])
                    lhs_flag=re.compile('(\n|^)(((?:[^#]*),)*'+pp+',((?:[^#]*),)*##)',re.IGNORECASE)
...
#    popup.title('Unique words in order frequency')
    text4.load(tmp+"uniqwords.txt")  ## was text5
